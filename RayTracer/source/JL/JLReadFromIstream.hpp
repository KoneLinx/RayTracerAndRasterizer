...
namespace JL
{
	...
	namespace Read_detail
	{
		...
		void Probe(std::basic_istream<Char>& is, ReadType& var)
		{
			std::string input{};
			is >> input;
	...
	template<typename ReadType, typename Char>
	typename Read_detail::StringToResult_t<ReadType>& Read(std::basic_istream<Char>& is, ReadType& val)
	{
		Read_detail::Probe<ReadType, Char>(is, val);
		return val;
	}
	
	template<typename ReadType, typename Char>
	typename Read_detail::StringToResult_t<ReadType> Read(std::basic_istream<Char>& is)
	{
		ReadType val{};
		Read_detail::Probe<ReadType, Char>(is, val);
		return val;
