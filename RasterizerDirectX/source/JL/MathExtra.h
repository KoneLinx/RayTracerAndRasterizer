...
namespace JL
{
	...
	*/
	...
	template<typename T>
	inline Point<3, T>& operator *= (Point<3, T>& v, const Matrix<4, 4, T>& m)
	{
	...
	inline Object<4, T>& operator *= (Object<4, T>& v, const Matrix<4, 4, T>& m)
	{
		...
		const T z{ m(2, 0) * v.x + m(2, 1) * v.y + m(2, 2) * v.z + m(2, 3) * v.w };
	...
	template<typename T>
	inline Matrix<3, 3, T> MakeRotationUp(const Vector<3, T>& newX, const Vector<3, T>& up)
	{
	...
	inline Matrix<3, 3, T> MakeRotation(const Vector<3, T>& axis, const T& angle)
	{
		...
		const Matrix<3, 3, T> unit{ MakeRotationUp(axis, up) };
	...
	template<typename T>
	inline Matrix<3, 3, T> MakeRotation(Vector<3, T> const& from, Vector<3, T> const& to)
	{
		return MakeRotation(Inverse(MakeRotation(from)) * to);
	}
	...
	template<int N, typename T>
	Matrix<N, N, T> MakeScale(T const& scale)
	{
		return Matrix<N, N, T>::Identity() * scale;
	}
	...
	template<typename T, typename R1, typename ... R, template<int, typename> typename Object, typename = std::enable_if_t<sizeof...(R) != 0>>
	Vector<3, T>& Scale(Object<3, T>& vector, const Object<3, R1>& scalar1, const Object<3, R>& ... scalars)
	{
		vector.x *= scalar1.x * ( scalars.x * ... );
		vector.y *= scalar1.y * ( scalars.y * ... );
		vector.z *= scalar1.z * ( scalars.z * ... );
		return vector;
	}


	template<int N, typename T, typename T2, template<int, typename> typename Object>
	Vector<N, T>& Scale(Object<N, T>& vector, const Object<N, T2>& scale)
	{
		for (size_t i{}; i < N; ++i)
			vector[i] *= scale[i];
		return vector;
	...
	template<int N, typename T>
	T Distance(Vector<N, T> const& a, Vector<N, T> const& b)
	{
		return Elite::Distance(
			reinterpret_cast<Point<N, T> const&>(a),
			reinterpret_cast<Point<N, T> const&>(b)
		);
	}
